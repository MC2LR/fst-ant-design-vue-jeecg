  ...
  <div class="standard-table">
    <div class="alert">
      <a-alert type="info" :show-icon="true">
        <div slot="message">
          選択済み&nbsp;<a style="font-weight: 600">{{ selectedRows.length }}</a>&nbsp;&nbsp;
          ...
          <a style="margin-left: 24px" @click="onClearSelected">空をクリア</a>
        </div>
      </a-alert>
    </div>
    <a-table
...
<script>
  export default {
    ...
    props: {
      ...
      /**
       * データローディング関数、戻り値はPromiseでなければなりません
       *デフォルトではdataパラメータを渡さなければなりません；
       *    ローカルデータレンダリングテーブルを使用すれば、ビジネスコードにローカルデータを取得してPromiseとして包装すればいいです。
       *
       * currentDataは表に現在レンダリングされているデータを外部に露出するために使用されます，
       * ビジネス開発においても、直接にcurrentDataを変更して、テーブルを再レンダリングすることができます（クライアントの並べ替え、データフィルタリングなどのシーンのみを推奨します）
       */
      data: {
        type: Function,
        required: true
