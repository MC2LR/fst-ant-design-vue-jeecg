    ...
    <a-spin :spinning="confirmLoading" v-if="editStatus">
      <a-form :form="form">
        ...
        <!-- <a-form-item
          ...
          label="企业名称"
          hasFeedback>
          <a-input placeholder="请选择企业" v-decorator="['enterpriseId', {rules: [{ required: true, message: this.$t('writeShopName') }]}]"
                   :readOnly="disableSubmit"/>
                   <a-select v-decorator="['enterpriseId', {}]" placeholder="请选择企业" @change="getShop">
                <a-select-option
        ...
        <a-form-item
          ...
          label="店铺名称"
          hasFeedback>
          <a-input placeholder="请选择店铺" v-decorator="['shopId',{}]" :readOnly="disableSubmit"/>
        </a-form-item> -->
        <!-- <a-form-item
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
          :label="$t('areaDefNmae')"
          hasFeedback>
          <a-input placeholder="请选择区域类型" v-decorator="['monitorAreaId',{}]" :readOnly="disableSubmit"/>
          <a-select v-decorator="['monitorAreaId', {}]" :placeholder="$t('selectAreaDef')">
                <a-select-option
                  v-for="(shop, index) in areadefiniton"
                  :key="index"
                  :value="shop.id"
        ...
        <a-form-item
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
          :label="$t('areaDeatilCode')"
          hasFeedback>
          <a-input v-decorator="[ 'monitorAreaDetailsCode',{}]" :readOnly="disableSubmit"/>
        </a-form-item>
        <a-form-item
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
          :label="$t('areaDeatilName')">
          <a-input style="width: 200px" v-decorator="[ 'name', {}]"/>
        </a-form-item>
      </a-form>
    </a-spin>
  </a-modal>
  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          add: "/test/mstMonitorAreaDetails/add",
          edit: "/test/bank/edit",
    ...
    methods: {
      ...
      edit(record, v) {
        ...
        if (record.id) {
          ...
          this.$nextTick(() => {
            this.form.setFieldsValue(pick(this.model,'name','monitorAreaDetailsCode'))
          });
        } else {
          this.addStatus = false;
          this.editStatus = true;
      ...
      handleOk() {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            formData.monitorAreaId = this.parentId;
